      ...
      PROGRAM MURN
C 
C     modified from murn.f : D.C. Univ of Wyoming, 10.23.2017
...
c
      write(iout,500)
  500 format(8x,'alat(A)',10x,'energy(Ryd)',12x,'alat(bohr)', 10x,
     &'energy(H)',10x,'energy(eV)')
      write(iout,1120)
      do i=1,nr_alat
...
c	alatt=alatt*ula
	...
	etotry=etot/convyy
	write(iout,502) alatt, etotry, alatt/ula, etotry/2,etot
      enddo
  502 format(2(f15.5,f20.7),f20.7)
c
      STOP
      END
C - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
C OF VOLUME.
C
C   BIRCH MURNAGHAN EOS : D.C. 10.23.2017
C
C INPUT DATA:
C
...
C
      ...
      END IF
C
C      ETOT = E0 - B0*VOL0/B0PRIM *
C     1 (((VOL/VOL0)**(1.D0-B0PRIM)-1.D0)/(1.D0-B0PRIM)-VOL/VOL0+1.D0)
C     2 *ULA**3/CONVXX
       
      ETOT = E0 - 9.D0*B0*VOL0/16.D0 *
     1 ((VOL0/VOL)**(2.D0/3.D0)-1.D0)**2 *
     2 ((B0PRIM-6.D0)-(B0PRIM-4.D0)*((VOL0/VOL)**(2.D0/3.D0)))
     3 *ULA**3/CONVXX
